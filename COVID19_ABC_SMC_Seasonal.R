...
# N0, age_sh, age_rt, beta, f_E, delta, age_bg, rho, epsilon0, epsilon1, phi
lm.low<-c(1*10^6,  0.1, 0.1, 0, 0, 0,  0, 0.5, 0, 0, 0)
lm.upp<-c(60*10^6,  20, 20, 1, 1,  1, 1, 1, 1, 1, 365)
  ...
  if(!file.exists(res.file)){
	...
	if(g<= G){
		if(g==1){
			...
			epsilon0_star<- runif(1,min=lm.low[9], max=lm.upp[9]) 
			epsilon1_star<- runif(1,min=lm.low[10], max=lm.upp[10]) 
			phi_star<- runif(1,min=lm.low[11], max=lm.upp[11]) 
			w.old<-rep(1/N,N)
		} else {
			w.old<-res$w[res$g==g-1]/sum(res$w[res$g==g-1])
			res.old<-res[res$g==g-1, c("N0", "age_sh", "age_rt", "beta", "f_E", 
			           "delta", "age_bg", "rho","epsilon0", "epsilon1", "phi")]
			...
			rho_star<-par[8]
			epsilon0_star<-par[9] 
			epsilon1_star<-par[10]
			phi_star<-par[11]
		}
      	#  Test if prior non zero
      	if(prior.non.zero(c(N0_star, age_sh_star, age_rt_star, beta_star,
      	         f_E_star, delta_star, age_bg_star, rho_star, epsilon0_star,
      	         epsilon1_star, phi_star))>0) {
    			# Set number of accepted simulations to zero
    			m<-0
    			for(j in 1:n){
    				D_star<-run_model_seasonal(N0_star, age_sh_star, age_rt_star, beta_star,
    				          f_E_star, delta_star, age_bg_star, rho_star, epsilon0_star,
    				          epsilon1_star, phi_star)     
    				sim<-sim+1
    				# Calculate distances 
    			...
    			if (m>0){
    				res.new<-c(N0_star, age_sh_star, age_rt_star, beta_star, f_E_star, 
    				  delta_star, age_bg_star, rho_star,  epsilon0_star, epsilon1_star, phi_star)  
