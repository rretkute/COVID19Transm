...
Seas<-function(t, epsilon0, epsilon1, phi){
  epsilon0*(1+ epsilon1*cos(2*pi*(t-phi)/365))
}        
        
run_model_seasonal<-function(N0, age_sh, age_rt, beta, f_E, delta, age_bg, rho, epsilon0, epsilon1, phi){
  ...
  while (t<t_end) {
    ...
    # S->E  
    lambda <- beta*( A %*% I_old) + pmax(Seas(t,epsilon0, epsilon1, phi),0)
